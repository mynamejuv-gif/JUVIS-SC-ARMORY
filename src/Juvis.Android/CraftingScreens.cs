...
public partial class MainActivity
{
    void BlueprintsScreen()
    {
        ...
        void Render() => Pager(results, armory.Catalog.Blueprints.Where(b => CatalogPresentation.BlueprintName(b) != null && Matches(query, CatalogPresentation.BlueprintName(b)!, string.Join(" ", b.Ingredients.Where(i => CatalogPresentation.HasName(i.Name)).Select(i => i.Name)))).OrderBy(b => b.Name).ToList(), b => {
            var card = Card(); card.AddView(Label(CatalogPresentation.BlueprintName(b)!, 19));
    ...
    {
        var blueprintName = CatalogPresentation.BlueprintName(b);
        if (blueprintName == null) { returnTo(); return; }
        Detail(blueprintName, $"Blueprint · {b.Version}\nCraft time: {b.Seconds / 60:0.#} minutes", returnTo);
        var generation = screenGeneration;
        var card = Card(); card.AddView(Label("Resources per craft", 19));
        foreach (var i in b.Ingredients.Where(i => CatalogPresentation.HasName(i.Name))) card.AddView(Label($"{i.Name}   {i.Quantity:0.###} {i.Unit}", 15));
        ...
        AddCommunitySource(blueprintName, b.Key, () => BlueprintScreen(b, returnTo));
        ...
        body.AddView(Button("✦ Ask Gemini about crafting", () => AskGemini($"Research crafting {blueprintName} in Star Citizen. Cached recipe patch: {b.Version}. Use this exact named variant; never substitute the base model or a different finish. Verify live availability, mission unlocks, crafting station requirements and quality/tier effects. Compare Citizen Starter Guide and Star Citizen Wiki for the stated patch. State missing or conflicting evidence explicitly and cite sources. Ingredients: " + string.Join(", ", b.Ingredients.Where(i => CatalogPresentation.HasName(i.Name)).Select(i => $"{i.Name} {i.Quantity} {i.Unit}")))));
    ...
    void CraftScreen()
    {
        ...
        var visiblePlan = armory.State.CraftPlan.Where(entry => armory.Catalog.Blueprints.Any(b => b.Id == entry.Key && CatalogPresentation.BlueprintName(b) != null)).ToList();
        foreach (var (id, quantity) in visiblePlan)
        {
            var b = armory.Catalog.Blueprints.FirstOrDefault(x => x.Id == id);
            var card = Card(); card.AddView(Label($"{CatalogPresentation.BlueprintName(b!)}  × {quantity}", 18));
            if (!b!.DefaultUnlocked && !armory.State.Blueprints.Contains(id)) card.AddView(Label("Blueprint unlock not marked owned", 12, cyan));
        ...
        if (armory.State.CraftPlan.Count > visiblePlan.Count) body.AddView(Label($"{armory.State.CraftPlan.Count - visiblePlan.Count} saved plan entr{(armory.State.CraftPlan.Count - visiblePlan.Count == 1 ? "y is" : "ies are")} hidden until a trustworthy catalog name is available. The saved data remains in exports.", 13, muted));
        body.AddView(Label("Total resources", 21));
        foreach (var i in Crafting.Requirements(armory.Catalog.Blueprints, armory.State.CraftPlan).Where(i => CatalogPresentation.HasName(i.Name)))
        {
            ...
            card.AddView(Button("Used in  ›", () => {
                ...
                foreach (var b in armory.Catalog.Blueprints.Where(b => b.Ingredients.Any(x => x.Id == i.Id)))
                    if (CatalogPresentation.BlueprintName(b) is { } name) body.AddView(Button(name, () => { BlueprintScreen(b, Draw); return Task.CompletedTask; }));
                return Task.CompletedTask;
            })); body.AddView(card);
        }
        body.AddView(Label("My blueprints", 21));
        var owned = armory.Catalog.Blueprints.Where(b => armory.State.Blueprints.Contains(b.Id) && CatalogPresentation.BlueprintName(b) != null).ToList();
        if (owned.Count == 0) body.AddView(Label("Mark a blueprint owned from its recipe page.", 14, muted));
        foreach (var b in owned.Take(30)) body.AddView(Button(CatalogPresentation.BlueprintName(b)!, () => { BlueprintScreen(b, Draw); return Task.CompletedTask; }));
