...
public partial class MainActivity
{
    ...
    void BlueprintsScreen()
    {
        ...
        void Render() => Pager(results, armory.Catalog.Blueprints.Where(b => (showIncompleteBlueprints || CatalogPresentation.HasName(b.Name)) && Matches(query, CatalogPresentation.BlueprintName(b), string.Join(" ", b.Ingredients.Select(i => i.Name)))).OrderBy(b => b.Name).ToList(), b => {
        ...
        var incomplete = armory.Catalog.Blueprints.Count(b => !CatalogPresentation.HasName(b.Name));
        if (incomplete > 0)
        {
            var toggle = new CheckBox(this) { Text = $"Show incomplete source records ({incomplete})", Checked = showIncompleteBlueprints };
            toggle.SetTextColor(ink); toggle.SetMinHeight(Dp(48));
            toggle.CheckedChange += (_, e) => { showIncompleteBlueprints = e.IsChecked; page = 0; Render(); };
            body.AddView(toggle);
        }
    ...
    {
        Detail(CatalogPresentation.BlueprintName(b), $"Blueprint · {b.Version}\nCraft time: {b.Seconds / 60:0.#} minutes", returnTo);
        if (!CatalogPresentation.HasName(b.Name)) body.AddView(Label("The source has no usable name for this blueprint. Its recipe is retained for review; availability is unverified.", 14, cyan));
    ...
    void CraftScreen()
    {
        ...
        foreach (var (id, quantity) in armory.State.CraftPlan)
        {
            ...
            var card = Card(); card.AddView(Label($"{(b == null ? id : CatalogPresentation.BlueprintName(b))}  × {quantity}", 18));
        ...
        foreach (var i in Crafting.Requirements(armory.Catalog.Blueprints, armory.State.CraftPlan))
        {
            ...
            card.AddView(Button("Used in  ›", () => {
                ...
                foreach (var b in armory.Catalog.Blueprints.Where(b => b.Ingredients.Any(x => x.Id == i.Id)))
                    body.AddView(Button(CatalogPresentation.BlueprintName(b), () => { BlueprintScreen(b, Draw); return Task.CompletedTask; }));
